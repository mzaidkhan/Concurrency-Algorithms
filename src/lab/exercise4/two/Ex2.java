...
class ListAdderThread extends Thread {
	...
	private int id;
	private int loop;
	private OptimisticLockedList linkedList;
	private Random ran = new Random();

	public ListAdderThread(int id, int loop, OptimisticLockedList linkedList) {
		this.id = id;
		this.loop = loop;
...
class ListRemoverThread extends Thread {
	...
	public ListRemoverThread(int id, int loop, OptimisticLockedList linkedList) {
		this.id = id;
		this.loop = loop;
		this.linkedList = linkedList;
	}
...
public class Ex2 {
	...
	public static void main(String[] args) {
		if (args.length == 1) {
			...
			OptimisticLockedList linkedList = new OptimisticLockedList();
			...
			try {
				...
				for (int i = 1; i <= n / 2; i++) {
					Thread t = new ListRemoverThread(n/2 + i, loop, linkedList);
					remThreads.add(t);
				}
